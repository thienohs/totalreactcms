...
exports.install = function() {
	...
	ROUTE('#admin/api/pages/',                    cms_process_page, ['post']); // ['*Page --> save', 'post']
...
function view_pages_preview() {
	...
	self.themeName = self.body.template;
...
// cms_process_page process the cms page with next support (Mike 13/12/2019)
function cms_process_page(){
	var self = this;

	const template = self.body.template;
	const pageUrl = self.body.url.replace(new RegExp('/', 'g'), '');
	const cms_body = self.body.body;

	$SAVE('Page', self.body, function(err, response) {
        if (err)
            self.invalid(err);
        else {
			self.success();
			// Now create a new next page with the template and the cms_body
			if (pageUrl.length > 0) {
				const templatePath = F.path.root('src/layouts/' + template + '.js');
				Fs.exists(templatePath, (exist) => {
					if (exist) {
						const generatedPagePath = F.path.root('pages/' + pageUrl.replace() + '.js');
						Fs.readFile(templatePath, "utf8", (err, templateFileContent) => {
							if (!err) {
								const generatedPageContent = templateFileContent.replace('{"<TEMPLATE_CONTENT>"}', cms_body);
								Fs.writeFile(generatedPagePath, generatedPageContent, "utf8", ()=>{
